...
def get_neighbors(node):
    ...
    wrapped_neighbors = [Vector2(neighbor.x % cell_number, neighbor.y % cell_number) for neighbor in neighbors]
...
def a_star(snake, fruit, obstacles):
    ...
    came_from = {}
    def heuristic(node):
        return min(abs(node[0] - goal_node[0]), cell_number - abs(node[0] - goal_node[0])) + min(abs(node[1] - goal_node[1]), cell_number - abs(node[1] - goal_node[1]))
    while open_list:
        ...
        for neighbor in get_neighbors(Vector2(current_node)):
            ...
            if neighbor in closed_list or neighbor in [tuple(body) for body in snake.body] or neighbor in obstacles:
                continue
...
class FRUIT:
    ...
    def randomize(self):
        while True:
            ...
            if a_star(self.snake, self, self.obstacles):
                break
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == SCREEN_UPDATE:
            path = a_star(main_game.snake, main_game.fruit, main_game.obstacles)
            if path is not None:
                next_node = path[0]
